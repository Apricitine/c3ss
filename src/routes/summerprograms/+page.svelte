...
<script lang="ts">
  ...
  import { Summer, type SummerDTO, type SummerFilterKey, type SummerFilter } from "$lib/scripts/summerprograms"
  import Search from "$lib/components/Search.svelte"

  import FilterSelect from "$lib/components/FilterSelectSummer.svelte"
  ...
  let summers = $derived(data.summers.map(Summer.from))
  ...
  let filterOptions = $derived.by(() => {
    ...
    for (const summer of summers) {
      for (const filter of summer.displayFilters()) {
  ...
  const getFiniteAwardValues = (summer: Summer) =>
    summer.estimated_cost.flatMap((prize) => {
  ...
  const getAwardRange = (summer: Summer) => {
    let lowest = Number.POSITIVE_INFINITY
    let highest = Number.NEGATIVE_INFINITY

    for (const prize of summer.estimated_cost) {
      const [min, max] = Array.isArray(prize.amount)
        ? prize.amount
        : [prize.amount, prize.amount]
  ...
  let awardBounds = $derived.by(() => {
    const values = summers.flatMap(getFiniteAwardValues)
  ...
  const openSummers = (summer: Summer, event: MouseEvent) => {
  ...
  const sortSummers = (term: string): Summer[] => {
    const query = term.trim()

    return [...summers]
      .map((summer) => ({
        summer,
        similarity: fuzzy(query, summer.name),
      }))
  ...
  const matchesFilters = (scholarship: Summer) => {
  ...
  let renderedSummers: Summer[] = $state(sortSummers(""))
...
{#if filtersOpen}
  <div class="filter-transition" transition:slide={{ duration: 180, axis: "y" }}>
    <FilterSelect
      ...
      resultCount={renderedSummers.length}
      totalCount={summers.length}
...
<section class="summer-grid">
  {#each renderedSummers as summer (summer.id)}
    <div
      class="summer-card-slot"
      class:source-hidden={showModal && activeSummer?.id === summer.id}
      aria-hidden={showModal && activeSummer?.id === summer.id}
    >
      <SummerProgramCard
        onclick={(event) => openSummers(summer, event)}
        name={summer.name}
        deadline={summer.formattedDeadline()}
        daysLeft={summer.daysUntil()}
        description={summer.description}
        estimated_cost={summer.estimated_costRange()}
