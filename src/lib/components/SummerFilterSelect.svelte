<script lang="ts">
  ...
  let lowerPercent = $derived(((minCost - rangeMin) / rangeSpan) * 100)
  let upperPercent = $derived(((maxCost - rangeMin) / rangeSpan) * 100)
  let costStep = $derived(Math.max(100, Math.round(rangeSpan / 10000) * 100))
  let canUseCostRange = $derived(rangeMax > rangeMin)
</script>

<section class="filter-panel" id="summer-filters" aria-label="Summer program filters">
  <div class="panel-header">
    <div>
      <p class="eyebrow">Filters</p>
      <p class="result-count">{resultCount} of {totalCount} programs</p>
    </div>
  ...
  <div class="filter-section">
    <div class="section-row">
      <p class="section-title">Cost Range</p>
      <p class="price-label">{formatCurrency(minCost)} to {formatCurrency(maxCost)}</p>
    </div>
    ...
    >
      <input
        ...
        style={`z-index: ${minCost >= maxCost && minCost > rangeMin ? 4 : 2}`}
        type="range"
        min={rangeMin}
        max={rangeMax}
        step={costStep}
        value={minCost}
        disabled={!canUseCostRange}
        aria-label="Minimum cost"
        aria-valuetext={formatCurrency(minCost)}
        oninput={(event) => onMinCostChange(Number(event.currentTarget.value))}
      />
      <input
        ...
        aria-valuetext={formatCurrency(maxCost)}
        oninput={(event) => onMaxCostChange(Number(event.currentTarget.value))}
      />
    </div>
